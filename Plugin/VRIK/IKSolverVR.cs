	...
	public class IKSolverVR : IKSolver
	{
		...
		private void Solve()
		{
			...
			if (this.locomotion.weight > 0f)
			{
				...
				this.rootVelocity += (vector - this.rootBone.solverPosition) * Time.unscaledDeltaTime * 10f;
				Vector3 b = V3Tools.ExtractVertical(this.rootVelocity, this.root.up, 1f);
				this.rootVelocity -= b;
				Vector3 vector2 = this.rootBone.solverPosition + this.rootVelocity * Time.unscaledDeltaTime * 2f * this.locomotion.weight;
				vector2 = Vector3.Lerp(vector2, vector, Time.unscaledDeltaTime * this.locomotion.rootSpeed * this.locomotion.weight);
				this.rootBone.solverPosition = vector2;
				float d3 = num + num2;
				this.bodyOffset = Vector3.Lerp(this.bodyOffset, this.root.up * d3, Time.unscaledDeltaTime * 3f);
		...
		private Vector3 GetPelvisOffset()
		{
			...
			this.lastOffset = Vector3.Lerp(this.lastOffset, Vector3.zero, Time.unscaledDeltaTime * 3f);
			a += Vector3.ClampMagnitude(this.lastOffset, 0.75f);
			a.y = this.spine.pelvis.solverPosition.y;
			this.lastOffset = Vector3.Lerp(this.lastOffset, a - this.spine.pelvis.solverPosition, Time.unscaledDeltaTime * 15f);
		...
		public class Footstep
		{
			...
			{
				this.stepTo = Vector3.Lerp(this.stepTo, p, Time.unscaledDeltaTime * speed);
				this.stepToRot = Quaternion.Lerp(this.stepToRot, rootRotation * this.footRelativeToRoot, Time.unscaledDeltaTime * speed);
			...
			public void UpdateStanding(Quaternion rootRotation, float minAngle, float speed)
			{
				...
				if (num > minAngle)
				{
					this.rotation = Quaternion.RotateTowards(this.rotation, quaternion, Mathf.Min(Time.unscaledDeltaTime * speed * (1f - this.supportLegW), num - minAngle));
				}
			}
			...
			public void Update(InterpolationMode interpolation, UnityEvent onStep)
			{
				...
				this.stepProgress = Mathf.MoveTowards(this.stepProgress, 1f, Time.unscaledDeltaTime * this.stepSpeed);
		...
		public class Locomotion
		{
			...
			public void Solve(IKSolverVR.VirtualBone rootBone, IKSolverVR.Spine spine, IKSolverVR.Leg leftLeg, IKSolverVR.Leg rightLeg, IKSolverVR.Arm leftArm, IKSolverVR.Arm rightArm, int supportLegIndex, out Vector3 leftFootPosition, out Vector3 rightFootPosition, out Quaternion leftFootRotation, out Quaternion rightFootRotation, out float leftFootOffset, out float rightFootOffset, out float leftHeelOffset, out float rightHeelOffset)
			{
				...
				this.comVelocity = ((Time.unscaledDeltaTime <= 0f) ? Vector3.zero : ((this.centerOfMass - this.lastComPosition) / Time.unscaledDeltaTime));
